...
The main differentiator from traditional textbook publication methods are the
products's micro-writing abilities.
A traditional textbook is written by one expert, using a defined curriculum that
fits the needs of the author.
The curriculum is a generic version of a multitude of classes that is somewhat useful for many professors,
rather than very useful for few professors.
Instead, the platform gives individual professors working with their own curriculum to
create the textbook they need from the same kinds of components.
...
users to not think about the technical specificities of the product, but moreso
